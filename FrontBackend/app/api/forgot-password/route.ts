...
export async function POST(request: Request) {
  try {
    ...
    // Use reset_code column — does NOT touch verification_code
    await sql`UPDATE users SET reset_code = ${code} WHERE email = ${cleanEmail}`;
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;padding:40px 32px;background:#0f172a;border-radius:20px;">
          ...
          <h2 style="color:#f1f5f9;font-size:22px;text-align:center;margin-bottom:10px;font-weight:800;">Şifre Sıfırlama</h2>
          ...
          <div style="background:#1e293b;border:2px solid #7c3aed;border-radius:16px;padding:32px;text-align:center;margin-bottom:36px;">
            <div style="font-size:56px;font-weight:900;letter-spacing:16px;color:#8b5cf6;">
              ${code}
            </div>
          </div>
          <p style="color:#475569;font-size:12px;text-align:center;line-height:1.6;">
            Bu kod 15 dakika geçerlidir. / This code is valid for 15 minutes.
          </p>
