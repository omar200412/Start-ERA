...
export async function POST(request: Request) {
  try {
    ...
    // ── Generic success message (used in all exit paths to prevent enumeration)
    ...
    if (userResult.rowCount === 0) {
      // User doesn't exist → return same generic message (anti-enumeration)
      return safeResponse;
    }
    ...
    // ── Save reset code + expiration to the database ────────────────────────
    await sql`
      UPDATE users
      SET reset_code = ${code},
    ...
    await resend.emails.send({
      ...
      headers: {
        "X-Entity-Ref-ID": `reset-${cleanEmail}-${Date.now()}`,
      },
      html: `
        <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;padding:40px 32px;background:#0f172a;border-radius:20px;">
          ...
          <div style="text-align:center;margin-bottom:36px;">
            <div style="display:inline-block;background:linear-gradient(135deg,#16a34a,#059669);color:white;font-size:26px;font-weight:900;padding:14px 28px;border-radius:14px;letter-spacing:-0.5px;">
              Start ERA
            </div>
          </div>
          ...
          <div style="background:#1e293b;border:2px solid #16a34a;border-radius:16px;padding:32px;text-align:center;margin-bottom:36px;">
            <div style="font-size:56px;font-weight:900;letter-spacing:16px;color:#22c55e;font-variant-numeric:tabular-nums;">
              ${code}
            </div>
          </div>
          ...
          <p style="color:#475569;font-size:12px;text-align:center;line-height:1.6;">
            Eğer bu isteği sen yapmadıysan, bu e-postayı görmezden gelebilirsin.<br/>
            If you did not request this, you can safely ignore this email.
          </p>

          <hr style="border:none;border-top:1px solid #1e293b;margin:28px 0;" />
          <p style="color:#334155;font-size:11px;text-align:center;margin:0;">© 2026 Start ERA · startera.io</p>
        </div>
      `,
    });
