...
function SparkleIcon() {
  return <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 3.214L13 21l-2.286-6.857L5 12l5.714-3.214z" /></svg>;
}
...
function parseApiResponse(data: any): { plan: PlanSection[]; scores: Scores | null } {
  ...
  if (Array.isArray(rawPlan)) {
    ...
    // Could be a JSON string of the array, or the full JSON object
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        // It's just the plan array
        plan = parsed;
      } else if (parsed && typeof parsed === "object") {
        // It's the full object {scores, plan}
        if (Array.isArray(parsed.plan)) plan = parsed.plan;
        if (parsed.scores && !scores) scores = parsed.scores;
      }
    } catch {
      // Unparseable — wrap raw text as one section
...
function CircleScore({ score, size = 56, isDark }: { score: number; size?: number; isDark: boolean }) {
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <span className="absolute text-sm font-black" style={{ color }}>{safeScore}</span>
    </div>
  );
}
  ...
  return (
    <div className="flex items-center gap-4">
      ...
      <div>
        ...
        <div className={"text-xs " + (isDark ? "text-gray-400" : "text-gray-500")}>{overallLabel}</div>
      </div>
    </div>
  );
}
...
export default function PlannerPage() {
  ...
  const txBg  = isDark ? "bg-gray-800 text-gray-100 placeholder-gray-500 border-gray-700 focus:bg-gray-900" : "bg-gray-50 text-gray-900 placeholder-gray-400 border-gray-200 focus:bg-white";
  ...
  if (!user) return (
    <div className={"flex h-screen items-center justify-center " + bg}>
      <div className="text-center">
        <p className={"opacity-50 font-bold mb-4 " + sub}>{lang === "tr" ? "Lütfen giriş yapın." : lang === "ar" ? "يرجى تسجيل الدخول." : "Please sign in."}</p>
  ...
  return (
    <div dir={isRTL ? "rtl" : "ltr"} className={"min-h-screen font-sans transition-colors duration-300 " + bg}>
      <style>{`
        ...
        .animate-pulse { background:linear-gradient(90deg,#e5e7eb 25%,#f3f4f6 50%,#e5e7eb 75%); background-size:200% 100%; animation:shimmer 1.6s ease-in-out infinite; }
      `}</style>
      ...
      <nav className={"sticky top-0 z-40 border-b backdrop-blur-md " + navBg}>
        <div className="max-w-6xl mx-auto px-6">
          <div className="flex items-center justify-between h-14">
            ...
            <div className="flex items-center gap-2">
              <button onClick={toggleLang} className={"text-xs font-bold px-2.5 py-1.5 rounded-lg border transition " + (isDark ? "border-gray-700 text-gray-300 hover:border-green-600" : "border-gray-200 text-gray-600 hover:border-green-500")}>{getLangLabel()}</button>
              <button onClick={toggleTheme} className={"p-2.5 rounded-lg border transition flex items-center gap-1.5 text-xs font-medium " + (isDark ? "border-gray-700 text-yellow-400 hover:bg-gray-800" : "border-gray-200 text-gray-600 hover:bg-gray-100")}>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        {showSkeleton && !planResult && (
          <div>
            <div className={"mb-6 p-4 rounded-xl border flex items-center gap-3 " + card}>
              <div className="w-5 h-5 border-t-2 border-green-600 rounded-full flex-shrink-0" style={{ animation:"spin 1s linear infinite" }} />
              <div>
        ...
        {planResult && !showSkeleton && (
          <div>
            ...
            <div className={"mb-6 p-4 rounded-2xl border flex items-center justify-between " + card}>
              <button onClick={resetWizard} className={"flex items-center gap-2 text-sm font-medium transition hover:text-green-600 " + sub}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-5">
                ...
                {/* Scores card — circles use SAME scores state as bars */}
                      ...
                      <div className="flex gap-5 flex-wrap">
                        {scoreKeys.map(key => (
                          <div key={key} className="flex flex-col items-center gap-1">
                            ...
                            <span className={"text-[10px] font-medium " + sub}>{scoreLabels[key]}</span>
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <div className="space-y-4 lg:sticky lg:top-20 lg:self-start">
                ...
                {/* Score bars — same scores state */}
                {scores && (
                  <div className={"rounded-2xl border p-5 " + card}>
                    <h4 className={"text-xs font-bold uppercase tracking-widest mb-4 " + sub}>{evalLabel}</h4>
                    <div className="space-y-3">
                      {scoreKeys.map(key => {
                        ...
                        return (
                          <div key={key} className="flex items-center gap-3">
                            <span className={"text-xs w-20 " + sub}>{scoreLabels[key]}</span>
                      ...
                      <div className={"border-t pt-3 flex items-center justify-between " + bdr}>
                        <span className={"text-xs font-bold " + sub}>{lang === "tr" ? "Genel" : lang === "ar" ? "الإجمالي" : "Overall"}</span>
          ...
          <div className="max-w-2xl mx-auto">
            <div className="mb-8">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className={"text-xs font-bold " + sub}>{t.step} {step} / 5</span>
              </div>
              ...
              <div className="flex justify-between mt-3">
                {[1,2,3,4,5].map(s => (
                  <div key={s} className={"w-2 h-2 rounded-full transition-colors " + (s <= step ? "bg-green-600" : (isDark ? "bg-gray-700" : "bg-gray-300"))} />
                ))}
              </div>
            </div>
