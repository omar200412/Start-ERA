...
export default function PlannerPage() {
  ...
  // Sync language in formData when lang changes
  useEffect(() => {
  ...
  const generatePlan = async () => {
    ...
    try {
      ...
      // Save to localStorage instantly (optimistic)
      const newProject = {
        id: Date.now(),
    ...
    return (
      <div className={`flex h-screen items-center justify-center ${darkMode ? 'bg-slate-950 text-white' : 'bg-slate-50'}`}>
        <div className="text-center">
          <p className="opacity-50 font-bold mb-4">{lang === "tr" ? "Lütfen giriş yapın." : lang === "ar" ? "يرجى تسجيل الدخول." : "Please sign in."}</p>
          ...
          <div className="relative w-full max-w-2xl">
            ...
            <div className={`p-8 md:p-12 rounded-[32px] shadow-2xl backdrop-blur-xl border ${darkMode ? "bg-slate-900/80 border-slate-800" : "bg-white/80 border-white/60"}`}>
              ...
              <div className="flex justify-between items-center mt-8">
                ...
                >
                  {step === 5 ? <><SparkleIcon />{t.start_magic}</> : <>{t.next} {lang === 'ar' ? '←' : '→'}</>}
                </button>
              </div>
            </div>
          </div>
