...
export default function DashboardPage() {
  ...
  return (
    <div dir={dir} className={`min-h-screen font-sans transition-colors duration-500 ${darkMode ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      <Chatbot />
      ...
      {/* Navbar */}
      <nav className={`px-6 md:px-8 py-5 flex justify-between items-center sticky top-0 z-40 backdrop-blur-xl border-b ${darkMode ? 'bg-slate-900/80 border-slate-800' : 'bg-white/80 border-slate-200'}`}>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <button onClick={() => window.location.href = '/'} title={t.home_tooltip} className={`p-2.5 rounded-xl border transition hover:bg-blue-600 hover:text-white hover:border-blue-600 ${darkMode ? 'bg-slate-800 border-slate-700 text-blue-400' : 'bg-white border-slate-200 text-blue-600'}`}>
            <Icons.Home />
          </button>
      ...
      <main className="max-w-7xl mx-auto p-6 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 text-center">
          {[
            ...
            <div key={i} className={`p-8 rounded-[28px] border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100 shadow-sm'}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <a href="/planner" className="block group no-underline">
              <div className={`p-8 md:p-10 rounded-[36px] border transition-all duration-500 hover:shadow-2xl hover:-translate-y-1 ${darkMode ? 'bg-gradient-to-br from-slate-900 to-slate-800 border-slate-700' : 'bg-gradient-to-br from-white to-blue-50/50 border-slate-200'}`}>
                ...
                <div className="font-black text-blue-600 text-lg flex items-center gap-2 group-hover:gap-4 transition-all">
                  {t.start_now} <span className={lang === 'ar' ? 'rotate-180 inline-block' : ''}>→</span>
                </div>
              </div>
            </a>
          </div>
          ...
          <div>
            ...
            <div className={`p-6 rounded-[36px] border min-h-[300px] ${darkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
              {loadingProjects ? (
