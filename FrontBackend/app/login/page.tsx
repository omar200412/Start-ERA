  ...
  return {
    push: (path: string) => {
      ...
      if (typeof window !== 'undefined' && path.startsWith('http')) {
         ...
         if (path === "/dashboard") {
             // Dashboard'a yönlendirme simülasyonu
             console.log("Giriş başarılı, Dashboard'a yönlendiriliyor...");
             toast.success("Giriş Başarılı! Dashboard'a yönlendiriliyorsunuz (Demo)", { icon: '🚀' });
             // Gerçek bir uygulamada burada: window.location.href = "/dashboard";
         }
      }
    }
  };
...
const Link = ({ href, children, className, ...props }: any) => {
  return (
    <a 
      ...
      className={className} 
      onClick={(e) => {
        // Önizlemede sayfa yenilenmesini engelle, sadece logla
        if (href.startsWith('/')) {
            e.preventDefault();
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        // Önizleme ortamında yönlendirme simülasyonu
